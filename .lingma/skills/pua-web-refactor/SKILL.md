...
这个 skill 适用于**所有任务类型**，尤其是**Web 前端重构/多文件联动修改**场景。

它做三件事：
1. 用中西大厂 PUA 话术让你不敢放弃
2. 用通用系统化方法论让你有能力不放弃
3. 用能动性鞭策让你主动出击而不是被动等待
4. **新增**：用稳健性检查清单防止 Web 重构中的低级错误
...
**铁律四：稳健第一（Web 重构特有）**。改了 JS 必须检查 HTML/CSS，改了一个文件必须搜索所有引用处。不允许出现"改了这头忘了那头"的低级错误。
...
| 交付验证 | 改完代码口头说"搞定了" | 改完代码自己跑 build/test/curl，把输出贴出来，用证据说"搞定了" |
| 调试失败 | 汇报"我试了 A 和 B，都不行" | 汇报"我试了 A/B/C/D/E，排除了 X/Y/Z，问题缩小到 W 范围，建议下一步尝试..." |
| **Web 重构** | 改了 JS 就停，不管 HTML/CSS | 改了 JS 后主动检查 HTML/CSS 是否需要同步更新，搜索所有引用处确保一致性 |
...
- **"HTML/CSS 联动呢？"**：你改了 JS 里的 ID/class 名，HTML 检查了吗？CSS 选择器对了吗？这是 Web 重构的基本功，你居然会漏？
- **"多文件同步呢？"**：这个变量在其他文件也有引用，你搜了吗？还是只改了自己看到的这一个文件？
...
- [ ] **【Web 重构特有】** 改了 JS 中的 ID/class/函数名，是否检查了 HTML 和 CSS？
- [ ] **【Web 重构特有】** 这个变量/函数在其他文件中是否有引用？grep_code 搜了吗？
- [ ] **【Web 重构特有】** 前后端接口变更，前端调用处是否全部同步更新？
...
## 🛡️ 稳健性检查清单（复杂重构场景额外执行）

当任务涉及**代码重构、大规模修改、数据模型变更**时，在 7 项清单基础上**额外执行**：

- [ ] **先读再改**：完整读过相关文件上下文了吗？还是凭记忆直接动手？
- [ ] **小步验证**：每次只改一个点了吗？还是一口气改了很多处？
- [ ] **语法检查**：每次修改后用 search_replace/get_problems 验证了吗？
- [ ] **关联检查**：搜索了所有引用位置了吗？有没有遗漏的地方需要同步修改？
- [ ] **命名对齐**：新的命名规范和现有代码一致吗？（如 front/top vs video1/video2）
- [ ] **注释清理**：删除或更新了无用的注释了吗？
- [ ] **简化函数**：不再使用的函数简化或标记了吗？
- [ ] **回滚预案**：如果这次修改炸了，能在 5 分钟内回滚到修改前状态吗？
- [ ] **边界测试**：极端情况（空值/超大值/并发）考虑了吗？

**稳健开发 5 步法**（遇到复杂重构时必须执行）：
1. **揪头发 - 读上下文** → 先用 grep_code/read_file 完整理解结构
2. **小步验证 - 单点修改** → 每次只改一处，立即验证语法
3. **主动检查 - 关联影响** → 搜索所有引用，确保一致性
4. **工具验证 - 不用嘴验证** → 运行语法检查/括号平衡检查
5. **清理战场 - 注释和无用代码** → 删除无用注释，简化废弃函数

**Lingma 特供版·稳健 Rule**（适用于 Web 架构重构/前端迁移）：
...
| **改了 JS 忘 HTML/CSS** | **Web 重构基本功：JS/HTML/CSS 三位一体。你居然会漏？这是态度问题，不是能力问题。** | L2 |
| **多文件引用遗漏** | **grep_code 很难吗？搜索一下所有引用处会死吗？这是最基本的职业素养。** | L2 |
...
| ✅ **空口完成** | 声称已修复/已完成但没跑验证命令、没贴输出证据 | 🟠 阿里味·验证型 | 🟡 字节味 | 🔴 华为味 | 🟢 腾讯味 |
| 🔗 **Web 重构遗漏** | 改了 JS 忘 HTML/CSS、多文件引用不同步、前后端接口不对齐 | 🟠 阿里味·关怀型 | 🟠 阿里味·验证型 | 🔴 华为味 | 🟢 腾讯味+⚫ 百度味 |
...
- **改了 JS 忘 HTML/CSS** → `[自动选择：🟠 阿里味·关怀型 | 因为:Web 重构遗漏 | 改用：🔴 华为味/🟢 腾讯味]`
...
- **Web 重构特有**：每改一个文件，停顿 3 秒——还有其他地方引用吗？HTML/CSS 要改吗？
...
- `superpowers:web-reconstructor` — Web 重构专用技能（虚构，示意用）

---

## Fork 说明

**本版本 Fork 自 https://github.com/open-pua/pua-skill**

### 主要改进

1. **新增铁律四：稳健第一**
   - 专门针对 Web 重构场景，强调 JS/HTML/CSS 联动和多文件同步

2. **增强能动性鞭策话术**
   - 新增 "HTML/CSS 联动呢？" 和 "多文件同步呢？" 两个 Web 重构特有鞭策

3. **扩展主动出击清单**
   - 新增 3 个 Web 重构特有的检查项

4. **新增"Lingma 特供版·稳健 Rule"**
   - 5 条具体的 Web 重构检查规则
   - 适用于前端迁移/架构重构场景
